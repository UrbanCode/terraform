...
import com.urbancode.uprovision.tasks.vmware.events.CloneVmCreatedEvent;
import com.urbancode.uprovision.tasks.vmware.events.TaskEventListener;
import com.urbancode.uprovision.tasks.vmware.events.TaskEventService;
    ...
    public void setIpListFromVmInfo() {
        ...
        for (int i=0; i<nicInfos.length; i++) {
            ...
            for (int j=0; j<nicInfoIpList.length; j++) {
                Ip4 newIp = new Ip4(nicInfoIpList[j]);
                ipList.add(newIp);
            }
        }
    }
